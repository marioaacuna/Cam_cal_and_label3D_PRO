function   calibration_extrinsic_Lframe(date, modify_lframe, num_cams,rootfolder)
...
extrinsic_filepath = fullfile(basedir ,'extrinsic');
...
%     filename =  [fileloc, filesep, 'cam',num2str(kk),'_lframe'];
    ...
    video_temp = VideoReader([filename '.mp4'],'CurrentTime',20); % Take the 20th frame (before set to 1st)
...
intrinsic_path = fullfile(basedir, filesep, 'intrinsic');
load([intrinsic_path filesep 'cam_intrinsics.mat']);
...
fprintf('%%%% Done extrinsic calibration l-frame %%%%\n')
