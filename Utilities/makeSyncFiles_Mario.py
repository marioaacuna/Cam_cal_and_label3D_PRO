...
def run (vidpath,fps, num_landmarks):
	# make sure num_landmarks is int
    num_landmarks = int(num_landmarks)

    _VALID_EXT = ["mp4", "avi"]

    #outpath = os.path.dirname(vidpath.rstrip(os.sep))
    outpath = os.path.join(vidpath, "sync")
    ...
    VI = get_vid_paths(vidpath)
    print("Found the following cameras: {}".format(VI))
    ...
    for d in VI:
        ...
        thisvid = imageio.get_reader(vids)
        cnt += thisvid.count_frames()
    ...
    fp = 1000 / int(fps)  # frame period in ms
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        run(vidpath=r'T:\Marta\test_Formalin\dannce\040822\MF_2epi', #'D:\\_test_label3D\\220414\\mouse1',
            fps=30,
            num_landmarks= 16
            )
        # vidpath = sys.argv[1]
